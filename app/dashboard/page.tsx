...
export default function DashboardPage() {
  ...
  return (
    <main className="min-h-screen w-full flex flex-col items-center justify-start p-3 sm:p-6 bg-[#070a12] text-white">
      ...
      <div className="w-full max-w-4xl space-y-4 sm:space-y-6 mt-2 sm:mt-12">
        
        {/* PREMIUM GLOWING HEADER BLOCK */}
        <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-r from-gray-950 to-[#0e1626] border border-white/5 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden transition-all duration-300">
          ...
          <div className="z-10">
            ...
            <h1 className="text-xl sm:text-3xl font-black bg-gradient-to-r from-white via-gray-200 to-gray-400 bg-clip-text text-transparent tracking-tight">
          ...
          <div className="flex flex-row items-center gap-2.5 z-10 w-full md:w-auto">
            <Link
              href="/labs"
              className="flex-1 md:flex-initial text-center whitespace-nowrap bg-indigo-600 hover:bg-indigo-500 active:scale-[0.98] px-4 sm:px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-wider text-white transition-all duration-200 shadow-lg shadow-indigo-600/20 border border-indigo-400/20 flex items-center justify-center gap-2"
            >
              <span>+ Launch Active Exam</span>
            </Link>
            <Link href="/" className="bg-white/5 hover:bg-white/10 active:scale-[0.98] px-3.5 sm:px-4 py-3 rounded-xl border border-white/5 text-xs font-bold uppercase tracking-wider transition-all duration-200 text-gray-300 text-center">
              Home
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          <div className="p-4 sm:p-5 rounded-2xl bg-[#0f1626]/60 backdrop-blur-md border border-white/5 shadow-xl flex flex-col justify-between relative group hover:border-white/10 transition-all duration-300">
            <div>
              <span className="text-[9px] sm:text-[10px] uppercase tracking-widest text-gray-400 font-bold block mb-1">Total Intakes</span>
              <span className="text-3xl sm:text-4xl font-black font-mono text-indigo-400 tracking-tight transition-transform duration-300 group-hover:scale-105 inline-block origin-left">{shiftHistory.length}</span>
            </div>
            <p className="text-[9px] sm:text-[10px] text-gray-500 mt-2 sm:mt-3 border-t border-white/5 pt-2">Cases evaluated this shift</p>
          </div>
          <div className="p-4 sm:p-5 rounded-2xl bg-[#0f1626]/60 backdrop-blur-md border border-white/5 shadow-xl flex flex-col justify-between relative group hover:border-white/10 transition-all duration-300">
            <div>
              <span className="text-[9px] sm:text-[10px] uppercase tracking-widest text-gray-400 font-bold block mb-1">Diagnostic Accuracy</span>
              <span className={`text-3xl sm:text-4xl font-black font-mono tracking-tight transition-transform duration-300 group-hover:scale-105 inline-block origin-left ${averageScore >= 70 ? "text-emerald-400" : "text-amber-400"}`}>{averageScore}%</span>
            </div>
        ...
        <div className="rounded-2xl bg-[#0f1626]/40 backdrop-blur-md border border-white/5 shadow-2xl p-4 sm:p-5 overflow-hidden">
          ...
          {shiftHistory.length === 0 ? (
            <div className="text-center py-12 sm:py-16 border border-dashed border-white/5 rounded-2xl bg-black/10 px-4">
            ...
            <div className="divide-y divide-white/5 max-h-80 overflow-y-auto pr-1 space-y-2 pt-1 scrollbar-thin scrollbar-thumb-slate-800">
              {shiftHistory.map((log) => (
  <button
    key={log.id} 
    onClick={() => openModal(log)}
    className="w-full text-left py-3 px-3 sm:px-4 rounded-xl bg-black/10 hover:bg-white/[0.02] border border-white/[0.01] hover:border-white/[0.08] flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 text-sm transition-all duration-200 ease-out group focus:outline-none hover:translate-x-0.5 active:scale-[0.995]"
  >
    ...
    <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
      {log.timestamp && (
        <span className="text-[10px] font-mono font-bold tracking-tight text-gray-500 bg-white/[0.02] border border-white/5 px-2 py-0.5 sm:py-1 rounded-md flex items-center gap-1.5 transition-colors group-hover:border-white/10 group-hover:text-gray-400">
      ...
      <div className="flex items-center gap-2">
        <div className={`font-mono font-black min-w-[55px] text-center px-2 py-1 rounded-lg border text-[11px] tracking-wider shadow-sm ${
          log.finalScore >= 70 
        ...
        <svg 
          className="w-3.5 h-3.5 text-gray-600 group-hover:text-gray-300 transition-all duration-200 transform group-hover:translate-x-0.5 shrink-0" 
          fill="none" 
          viewBox="0 0 24 24" 
          stroke="currentColor"
        >
      ...
      {selectedLog && activeModalLog && (
        ...
        >
          <div 
            className={`w-full max-w-md rounded-2xl bg-[#0f1626] border border-white/10 p-5 sm:p-6 shadow-2xl relative overflow-hidden space-y-4 ${
          ...
          >
            ...
            <div className="flex justify-between items-start gap-2">
              <div>
                ...
                <h3 className="text-lg font-black text-white truncate max-w-[240px]">{activeModalLog.patientName}</h3>
              </div>
              {activeModalLog.timestamp && (
                <span className="text-[10px] font-mono text-gray-400 bg-white/5 px-2 py-1 rounded-md border border-white/5 shadow-inner whitespace-nowrap">
            ...
            <div className="space-y-3 pt-1">
              ...
              <div className="p-3.5 rounded-xl bg-black/40 border border-white/5 flex items-center justify-between hover:border-white/10 transition-colors duration-300">
                <div>
                  ...
                  <span className="text-[11px] text-gray-500">Total metrics achieved</span>
                </div>
                <div className={`font-mono font-black text-sm sm:text-base px-3.5 py-1.5 rounded-xl border tracking-wider transition-all duration-300 ${
            ...
            <button 
              onClick={closeModal}
              className="w-full mt-2 bg-white/5 hover:bg-white/10 active:scale-[0.985] border border-white/5 py-3 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-150 text-gray-300 flex items-center justify-center"
            >
              Close Record Review
            </button>
          </div>
