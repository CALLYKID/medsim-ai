        ...
        <div className="lg:col-span-7 flex flex-col h-full min-h-0 justify-between space-y-4">
          <div className="space-y-4 shrink-0">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-xl font-extrabold tracking-tight text-slate-100 flex items-center gap-2.5">
                  <span className="h-2.5 w-2.5 rounded-full bg-emerald-500 shadow-[0_0_12px_#10b981]" />
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-r from-slate-900/90 via-slate-900/60 to-slate-950/80 border border-slate-800/80 flex items-center gap-5 group hover:border-indigo-500/30 transition-all shadow-lg">
              <div className="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-slate-700/60 flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
                <img 
              ...
              <div className="flex-1 min-w-0">
                <span className="text-[10px] font-mono font-black text-indigo-400 uppercase tracking-widest block mb-0.5">
            ...
            {timeLeft !== null && (
              <div className={`p-3.5 rounded-2xl border flex items-center justify-between shadow-lg transition-all duration-500 ${
                !hasStarted 
                ...
                <div className="flex items-center gap-3">
                  <span className="relative flex h-2.5 w-2.5">
                    ...
                    <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${hasStarted && timeLeft <= 60 ? "bg-rose-500" : hasStarted ? "bg-indigo-500" : "bg-slate-600"}`}></span>
                  </span>
                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">OSCE Session Timer</span>
                    <span className="text-xs font-semibold text-slate-200 mt-0.5">
                      {!hasStarted ? "Awaiting Consultation Start" : "Active Evaluation Station"}
                    </span>
                  </div>
                </div>
                ...
                {!hasStarted ? (
                  <button
                    ...
                    className="cursor-pointer bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs uppercase tracking-wider px-5 py-2.5 rounded-xl transition-all shadow-lg active:scale-95"
                  >
                    ▶ Start Session
                  </button>
                ) : (
                  <div className={`text-xl font-mono font-black tracking-widest px-4 py-1.5 rounded-xl bg-slate-950/80 border shadow-inner ${
                    timeLeft <= 60 ? "text-rose-500 border-rose-500/30" : "text-indigo-400 border-slate-800"
                  }`}>
                    {formatTime(timeLeft)}
                  </div>
                )}
            ...
            <div className="p-4 rounded-2xl bg-slate-900/50 border border-slate-800 shadow-inner">
              <p className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-1.5">Presenting Chief Complaint</p>
              <p className="text-sm text-slate-100 font-medium leading-relaxed">{patient.disease.presentation?.chiefComplaint ?? "No data available."}</p>
            </div>
            ...
            <div className="relative flex items-center">
              <textarea
                ...
                placeholder={!hasStarted ? "Unlock clinical dashboard by initiating timer..." : isSessionEnded ? "Case finalized. Review score report." : "Type clinical inquiry to patient..."}
                rows={1}
                className="cursor-text w-full pl-4 pr-14 py-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500/80 focus:ring-2 focus:ring-indigo-500/20 disabled:opacity-40 disabled:cursor-not-allowed resize-none min-h-[50px] max-h-[120px] shadow-inner overflow-y-auto transition-all"
                onKeyDown={(e) => {
          ...
          <div className="flex-1 min-h-[200px] lg:min-h-[260px] relative flex flex-col overflow-hidden border-t border-slate-800/80 pt-3">
            <div 
            ...
            >
              {isResponding && (
                <div className="p-3.5 rounded-2xl bg-indigo-950/30 border border-indigo-500/30 text-indigo-400 text-xs font-semibold flex items-center gap-3 animate-pulse">
              ...
              {messages.map((m, idx) => (
                <div key={m.id} className={`p-4 rounded-2xl border transition-all duration-300 shadow-sm ${
                  m.role === "user" 
                    ? "bg-indigo-950/15 border-indigo-500/20 ml-10" 
                    : "bg-slate-900/80 border-slate-800/90 mr-10"
                }`}>
            ...
            <div className="grid grid-cols-2 gap-2.5 shrink-0">
              {(["vitals", "heent", "chest", "abdomen", "neuro"] as const).map((examType) => {
                ...
                return (
                  ...
                  >
                    <span>{examType}</span>
                    <span className={`h-4 w-4 rounded border flex items-center justify-center text-[9px] font-black transition-all ${
              ...
              <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-3 shadow-xl animate-[fadeIn_0.3s_ease-out] overflow-y-auto max-h-[220px] scrollbar-thin scrollbar-thumb-slate-800">
                <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                  ...
                  <div className="text-right">
                    <span className="text-xl font-mono font-black text-emerald-400">{score}/100</span>
                  </div>
                </div>
                ...
                <div className="text-xs text-slate-200 leading-relaxed bg-slate-950/90 p-3 rounded-xl border border-slate-800 shadow-inner">
                  {feedback}
                </div>
                
                <button 
                  onClick={admitPatient} 
                  className="cursor-pointer w-full bg-slate-800 hover:bg-slate-700 text-slate-100 font-bold text-xs uppercase tracking-wider py-3 rounded-xl border border-slate-700/80 transition-all active:scale-98 shadow-md"
                >
